...
func (p *ProductController) SubmitProduct() {
	...
	if err != nil {
		logs.Warn("invalid product_total, err: %v", err)
		errorMsg = fmt.Sprintf("invalid product_total, err: %v", err)
		return
	}
	productStatus, err := p.GetInt("product_status")
	if err != nil {
		logs.Warn("invalid product_status, err: %v", err)
		errorMsg = fmt.Sprintf("invalid product_status, err: %v", err)
		return
	}
	productModel := model.NewProductModel()
	...
	logs.Debug("product name[%s], product total[%d], product_status[%v]", productName, productTotal, productStatus)
}
